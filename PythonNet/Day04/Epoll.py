...
p = epoll()  # 创建epoll对象 # 改此
p.register(server, EPOLLIN) # 注册IO 关注server的pollin事件 # 改此
# 循环监控
while True:
    ...
    for fd,e in events:#循环处理
        ...
        if fd == server.fileno():#监听socket
            ...
            print("收到连接:",addr)
            p.register(client, EPOLLIN|EPOLLET)#注册事件 # 改此 |EPOLLET 边缘触发 只通知一次  如果客户端再次发送信息 才会通知 但是不会对已有的第一批请求不会再次通知 这里做或运算是因为
            #将新建的socket放入映射表
            fdmap[client.fileno()]=client
        elif e & EPOLLIN:#判断event是否有读事件 # 改此
            #pass
            data = sock.recv(1024)
            if not data:
