...
# @app.route("/") FLASK中的路由定义，主要匹配用户的访问路径 ‘/’表示的是整个网站的根路径 也就是spring 里面的mapping
# def index() 表示匹配上访问路径之后路的处理程序 也就是Java中的action 也就是MTV 中的 V MVC的C  视图处理函数必须要有返回值 这个例子中的返回值是字符串 表示要响应给客户端浏览器的内容
# @app.route('/')  # Flask(__name__)返回值叫啥 @后就叫啥 不一定要叫app
...
# 声明带两个参数的路由
@app.route('/show/<name>/<int:age>')  # 如果不用int: 使用的时候int(age) 用int:传进来的时候就转好
...
# 传进来都视为str 你需要当成int 自己转 或者转成float 随便
...
def url():
    ...
    # 将路径构建成a标记进行相应
    url_show = url_for('admin_show',
                       name="wangwc")  # 获取admin_show的route 并返回 不是返回  "这是admin下login下的form下的url下的show的访问路径,传递进来的参数为%s"  而是返回 route "/admin/login/form/url/show/<name>"
...
if __name__ == "__main__":
    ...
    #  debug=True 以调试的方式启动服务 如果你修改代码 会自动重启 如果代码错误 会把错误 直接报错到浏览器上
